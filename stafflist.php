...
*     Utilities
***********************************************************************************/
use stafflistUtils\SimpleCSV;
use stafflistUtils\SimpleXLS;
use stafflistUtils\SimpleXLSX;
use stafflistUtils\SimpleXLSXGen;

/***********************************************************************************
*     Constants
***********************************************************************************/
define( 'STAFFLIST__PLUGIN_DIR',	plugin_dir_path( __FILE__ ) );
define( 'RECORDS_PER_PAGE', 		20);
define( 'STAFFLIST_URL', 			admin_url() . 'admin.php?page=stafflist');
$staffdb = 		$wpdb->base_prefix . "stafflist";
$staffmetadb = 	$wpdb->base_prefix . "stafflist_meta";

/***********************************************************************************
*     Setup Plugin > Create Table, and Check Roles
***********************************************************************************/
require_once(STAFFLIST__PLUGIN_DIR . "stafflist_setup.php"); global $wpdb;
...
function stafflist_admin_init() {
	wp_register_style( 'stafflist_admin', plugins_url('stafflist_admin.css?1', __FILE__) );
}
...
function stafflist_plugin_options(){
	...
	if(isset($_GET['remove']) && is_numeric($_GET['remove'])){
		if(check_admin_referer()){
			...
			$r = (int) trim($_GET['remove']);
			$cr = $wpdb->get_var($wpdb->prepare($q,$r));
			if($wpdb->get_var($wpdb->prepare($q,$r)==1)){
				$q = "DELETE FROM $staffdb WHERE id=%s"; 
				$wpdb->query($wpdb->prepare($q,$r));
			}
		}
	}
	...
	$s = 	$_GET['s'] = (isset($_GET['s']) && in_array(trim($_GET['s']),array("last","first","dept","email","last-","first-","dept-","email-")) ? (string) trim($_GET['s']) : "last");
	$sort = $sorts[str_replace("-","",$s)];
	$dir = 	(strstr($s,"-")?"DESC":"ASC");

	//handle search (use mb_strtolower, where available)
	if(isset($_GET['search']) && (string) trim($_GET['search'])!=""){
		
		$w = (function_exists('mb_strtolower') ? 
				mb_strtolower(	sanitize_text_field($_GET['search']),'utf8') :
				strtolower(		sanitize_text_field($_GET['search'])));
		$ws = '%'.$wpdb->esc_like($w).'%';
		$where = "WHERE	LOWER(lastname) 	LIKE %s OR
						LOWER(firstname) 	LIKE %s OR
						LOWER(department)  	LIKE %s OR
						LOWER(email) 		LIKE %s";
		$params = array($ws,$ws,$ws,$ws);
		
		//add nonstandard rows to search
		$nonstd = getNonstandardRows(1);
		if(!empty($nonstd)){
			foreach($nonstd as $k=>$v){
				$where.= " OR LOWER({$k}) LIKE %s ";
				$params[] = $ws;
			}
		}
	...
	//get count, first
	$q = 		"SELECT count(id) FROM $staffdb {$where}";
	$sql =		$wpdb->prepare($q,$params);
	$count =  	$wpdb->get_var($sql);

	//handle paging
	$pg = array((int) $count, (int) $rows, (int) ceil($count/$rows));
	$p = $_GET['p'] = $pg[3] = (!isset($_GET['p']) || !is_numeric($_GET['p']) || intval($_GET['p'])<2 || intval($_GET['p'])>$pg[2] ? 1 : intval($_GET['p']));
	...
	$sql = 		$wpdb->prepare($q,$params);
	$staff = 	$wpdb->get_results($sql, ARRAY_A);									 		//myprint_r($staff);
			...
			<h1>StaffList</h1>
		  	<div id='stafflist_instructions' data-expanded='0'>
			  	...
			  	You can insert the StaffList directory into any WordPress page or post using the shortcode:<br /> 
			  		<tt>[stafflist]</tt><br /> 
				If you wish to change the default number of rows from {$rows} per page, use the shortcode <strong>rows</strong> attribute:<br /> 
			  		<tt style='color:#0073AA; font-weight:bold;'>[stafflist <span>rows=50</span>]</tt><br />
				To specify the initial sort column, hover the column names below to find its number, and use it in the shortcode <strong>sort</strong> attribute:<br /> 
			  		<tt style='color:#0073AA; font-weight:bold;'>[stafflist <span>sort=4</span>]</tt><br /> 
				If you wish to use just a subset of records, use the shortcode <strong>subset</strong> attribute:<br /> 
			  		<tt style='color:#0073AA; font-weight:bold;'>[stafflist <span>subset=&quot;department:marketing&quot;</span>]</tt><br />
			  	If you have a column in your StaffList called &quot;Building&quot; and wish to show only records from Building A and Building B, use:<br />
			  	...
			  	To limit this function to just enter (or just type), use:<br />
				...
				You can also configure StaffList to wait until a search value is entered before returning results:<br />
					<tt style='color:#0073AA; font-weight:bold;'>[stafflist <span>on=&quot;wait&quot;</span>]</tt>

		  		<div class='sltabs'> 			
		  			<div class='sltab' style='float:right;'>Shortcodes</div>
		  		</div>
		  	</div>
		 ";
...
***********************************************************************************/
	...
	echo "<input type='text' id='searchdirectory' name='searchdirectory' value='".esc_attr(stripslashes($w))."' placeholder='Search Directory' />";

	global 	$stafflisturl; 
			$stafflisturl = STAFFLIST_URL . ($w ? "&search=".esc_attr($w) : "") . ($s ? "&s=".esc_attr($s) : "");
	...
	echo "<thead id='stafflisthead'><tr>
			<th><a href='{$stafflisturl}&s=last' title='Sort by Last Name A-Z' class='sort_a ".($s=='last'?'selected':'')."'><span>Ascending</span></a>
				<a title='For initial sort by Last Name, use &quot;sort=2&quot; in your shortcode.'>Last Name</a>
				<a href='{$stafflisturl}&s=last-' title='Sort by Last Name Z-A' class='sort_d ".($s=='last-'?'selected':'')."'><span>Descending</span></a>
			</th><th>&nbsp;</th>
			<th><a href='{$stafflisturl}&s=first' title='Sort by First Name Ascending' class='sort_a ".($s=='first'?'selected':'')."'><span>Ascending</span></a>
				<a title='For initial sort by First Name, use &quot;sort=1&quot; in your shortcode.'>First Name</a>
				<a href='{$stafflisturl}&s=first-' title='Sort by First Name Descending' class='sort_d ".($s=='first-'?'selected':'')."'><span>Descending</span></a>
			</th><th>&nbsp;</th>
			<th><a href='{$stafflisturl}&s=dept' title='Sort by Department Ascending' class='sort_a ".($s=='dept'?'selected':'')."'><span>Ascending</span></a>
				<a title='For initial sort by Department, use &quot;sort=5&quot; in your shortcode.'>Department</a>
				<a href='{$stafflisturl}&s=dept-' title='Sort by Department Descending' class='sort_d ".($s=='dept-'?'selected':'')."'><span>Descending</span></a>
			</th><th>&nbsp;</th>
			<th><a href='{$stafflisturl}&s=email' title='Sort by Email Address Ascending' class='sort_a ".($s=='email'?'selected':'')."'><span>Ascending</span></a>
				<a title='For initial sort by Email Address, use &quot;sort=4&quot; in your shortcode.'>Email Address
				<a href='{$stafflisturl}&s=email-' title='Sort by Email Address Descending' class='sort_d ".($s=='email-'?'selected':'')."'><span>Descending</span></a>
			</th><th>&nbsp;</th>
	...
	foreach($staff as $k=>$v){
		$del = "<a href='".wp_nonce_url("{$stafflisturl}&remove={$v['id']}&p={$p}&s={$s}")."' class='remove'
				   onclick='javascript:if(!confirm(\"Are you sure you want to delete this staff record?\")) return false;'
				   title='Permanently Delete This Staff Record' target='_self'></a>";
		$i++;
		
		echo "<tr class='row' id='staff_{$v['id']}'>
		<td><input type='text' id='lastname:{$v['id']}' value='".esc_attr($v['lastname'])."' autocomplete='Off' /></td><td></td>
		<td><input type='text' id='firstname:{$v['id']}' value='".esc_attr($v['firstname'])."' autocomplete='Off' /></td><td></td>
		<td><input type='text' id='department:{$v['id']}' value='".esc_attr($v['department'])."' autocomplete='Off' /></td><td></td>
		<td><input type='text' id='email:{$v['id']}' value='".esc_attr($v['email'])."' autocomplete='Off' /></td><td></td>
		<td><input type='text' id='phone:{$v['id']}' value='".esc_attr($v['phone'])."' autocomplete='Off' /></td><td></td>";
		for($i = 1; $i <= $activeCols; $i++){
			$col = $i+5;
			$key = "col{$col}";
			echo "<td><input type='text' id='col{$col}:{$v['id']}' value='".esc_attr($v[$key])."' autocomplete='Off' /></td>";
			...
			<tfoot>
				<tr>
					<td colspan='2'>You may import a [ {$doccsv}, {$docxls}, {$docxlsx} ] with up to 14 additional columns apart from the standard ones: <strong>firstname, lastname, email, phone, department</strong>
								    <p><strong>Using Email Addresses and Phone Numbers</strong><br />Values in the <strong>email</strong> column will be presented with the standard <a href='https://www.w3schools.com/tags/tag_address.asp' target='_blank' title='The HTML Address Tag'><strong>mailto:</strong></a> markup, 
									and values in the <strong>phone</strong> (and/or <strong>mobile</strong>, <strong>office</strong>, <strong>cell</strong>) column will be presented with <a href='https://www.w3schools.com/tags/tryit.asp?filename=tryhtml_link_phoneto' target='_blank' title='The HTML Phone-to Tag'><strong>tel:</strong></a> markup.<br />
									<p><strong>Using Links</strong><br />If your spreadsheet contains a column called <strong>URL</strong>, <strong>link</strong>, or <strong>profile</strong>, 
									it will be depicted as an icon {$urlhtml} with the column value used as a link.<br />
									<p><strong>Using Images</strong><br />Any imported value in a non-standard column that begins with &quot;<strong>img:</strong>&quot; will be replaced with an image tag in your staff directory.<br />
									These image tags will use the CSS class <em>stafflist_image_small</em> in the directory, and <em>stafflist_image_medium</em> in the contact card.<br /> 
									<strong>img:http://www.website.com/photo.jpg</strong> (for example)<br /><br />
									An example spreadsheet to test your StaffList data can be found here: <a href='http://www.era404.com/archive/stafflist.csv' title='ERA404 StaffList Sample Data' target='_blank'>http://www.era404.com/archive/stafflist.csv</a>
					</td>
				</tr>
			</tfoot>
			<tbody>
...
***********************************************************************************/
	echo "<div style='clear:both;'>
		...
		<h4>Table Columns</h4>These columns will be visible in the StaffList table, in the order you choose.
	...
	<h4>Card-only Columns</h4>These columns will only be visible in the contact card, but will be searchable like the others.
	...
	<h4>Hidden Columns</h4>These columns will not be visible anywhere, but will be searchable like the others.
...
function renderAdminPager($pg){
	$cur = $pg[3]; 
	$last = $pg[2];
	global $stafflisturl; //includes search & sort

	//one page or less?
	if($pg[0]<1 || $last<2) return("");
	//previous
	$html = ($cur > 1 ? "<p class='pager'><a href='{$stafflisturl}&p=".($cur-1)."'>Previous </a></p>" : "");
	//pages
	if($cur<=3){
	...
	//next
	$html .= (($cur < $pg[2] && $pg[2] > 1) ? "<p class='pager'><a href='{$stafflisturl}&p=".($cur+1)."'> Next</a></p>" : ""); //>
	//page numbering
...
function renderAdminPage($style, $page){
	...
	return("<p class='pager{$style}'>".(is_numeric($page)?"<a href='{$stafflisturl}&p={$page}'>{$page}</a>":$page)."</p>"); 
}
/***********************************************************************************
...
************************************************************************************
	...
	********************************************************************************/
	function ajax_build(){
		...
		$wait = (isset($_POST['wait']) && (string) trim($_POST['wait']) == "true" ? true : false);
		if(!isset($limit['sort']) || $limit['sort'] == "" || !preg_match('/^[\d]+-?$/', trim($limit['sort']))) $limit['sort'] = $cols[0]['id'];
		if(!isset($limit['page']) || !is_numeric($limit['page'])) $limit['page'] = 1;
		...
		//echo "WHERE: <br /><br />$where<br /><br />"; myprint_r($args);
		...
		//check sorting for numeric
		$isnumeric = ( $wpdb->get_var( "SELECT COUNT(*) FROM {$staffdb} WHERE {$sort} REGEXP '[^0-9]'" ) > 0 ? false : true);
		...
		if(isset($_POST['rows']) && is_numeric($_POST['rows']) && $_POST['rows'] > 0 && $_POST['rows'] <= 100) $rows = intval($_POST['rows']);
		
		//handle paging
		$pg = array((int) $count, (int) $rows, (int) ceil($count/$rows));
		$pg[3] = (int) (!isset($limit['page']) || $limit['page']<2 || $limit['page']>$pg[2] ? 1 : $limit['page']);
		$pg[4] = (int) ($pg[1]*$pg[3])-$pg[1];
		$pg[5] = (int) ($pg[4]+$pg[1])-1; if($pg[5]+1>$pg[0]) $pg[5]=($pg[0]-1);
		$limit['page'] = $pg;
		...
		$html = "<table class='stafflists'>\n".
				"\t<thead class='stafflisthead'><tr>\n".
					"\t\t<th style='width:36px;' class='hdrcard'>\n".
		...
		//build query
		if($wait && !$w) $pg[1] = 0;
		$q =   "SELECT * FROM {$staffdb} {$where} ORDER BY ". ($isnumeric ? "CAST({$sort} AS unsigned)" : $sort)." {$dir} LIMIT {$pg[4]}, {$pg[1]}"; //echo $q;
		...
		foreach($staff as $i=>$s){
			...
			$t = array(); //cleanup tooltip obj

			//tooltip object
			$t = createTooltipObject($s);
			...
			foreach($cols as $col){
				switch(strtolower($col['name'])){
					...
					//special case for phone to use tel hyperlink
						$urlcol = $col['col'];
						if(""!=trim($s[$urlcol]) && ""!=($cleannumber = preg_replace('/[^0-9]/', '', $s[$urlcol]))){
							$cleantitle = "Call ".esc_html(strip_tags($s['firstname']." ".$s['lastname']));
							$html .= "\t\t\t<td><a href='tel:{$cleannumber}' title='{$cleantitle}'>{$s[$urlcol]}</a></td>\n";
						} else {
							$html .= "\t\t\t<td>{$s[$urlcol]}</td>\n";
						}
						break;
					...
					//special case for url to use dashicon instead
						...
						break;
					//all others (including img tags)
					default: 
						if(substr($s[ $col['col'] ],0,4)=="img:"){
							$img = strip_tags(trim(substr($s[ $col['col'] ],4)));
							if(filter_var($img,FILTER_VALIDATE_URL)){
								$html .= "\t\t\t<td><img src='{$img}' class='stafflist_image_small' /></td>\n";
							} else {
								$html .= "\t\t\t<td></td>\n";
							}
						} else {
	...
	********************************************************************************/
	...
	function setup_staff_scripts() {
		...
		wp_enqueue_script( 'popper-script', plugins_url('/lib/tippyjs/popper.min.js', __FILE__), array('jquery'), '2.5.4' ); 	// jQuery will be included automatically
		wp_enqueue_script( 'tippy-script', plugins_url('/lib/tippyjs/tippy.min.js', __FILE__), array('jquery','popper-script'), '6.2.7' ); 		// jQuery will be included automatically
	}
	...
	********************************************************************************/
	$allcols = false;
	function createTooltipObject($s){
		global $wpdb, $staffmetadb, $std, $allcols;
		//columns to use
		if(!$allcols){
			$nonstd = $wpdb->get_results("SELECT id,name,colpos,active,
										  CASE WHEN id<6 THEN name ELSE concat('col',id) END AS col
										  FROM {$staffmetadb} WHERE active!=0 
										  ORDER BY colpos ASC", ARRAY_A);
		...
		foreach($allcols as $ck=>$ct){
			...
			if(array_key_exists($ck, $s) && trim($s[ $ck ])!="" && !is_null($s[ $ck ])){
				//handling for images
				if(substr($s[ $ck ],0,4)=="img:"){
					$t[ 'img' ] = strip_tags(trim(substr($s[ $ck ],4)));
				} else {
...
***************************************************************************************************
	...
	*	$_POST['data']['stafflist']['rename'] = array() => $rns
	***********************************************************************************************/
	function ajax_rename(){
		global $wpdb, $std, $messaging; $resp = array();
		parse_str($_POST['data'],$rnp); $rns = $rnp['stafflist']['rename'];

		//rename standard column labels
		foreach($std as $fd => $fdn){
			if(isset($rns[$fd])){
				$title = sanitize_option("stafflist_rename_{$fd}", (string) stripslashes(trim($rns[$fd])));
	...
	***********************************************************************************************/
	function getNonstandardRows($arrayonly=false){
		...
		foreach($nonstd as $k=>$col) {
			echo "\n\t\t\t<th>\n".
				 "\t\t\t\t<a title='For initial sort by ".esc_attr($col['name']).", use &quot;sort={$col['id']}&quot; in your shortcode.'>{$col['name']}</a>\n".
				 "\t\t\t</th>\n\t\t\t<th>&nbsp;</th>";
			$active++;
		}
		return($active);		
...
/**************************************************************************************************
*	Using PHPSpreadsheet to Parse Datafile
**************************************************************************************************/ 
function readCSVintoArray($file){
	...
	if(substr(strtolower($file),-4) == '.xls'){
	   	//importing from legacy (.xls) format
		require_once( STAFFLIST__PLUGIN_DIR . '/lib/shuchkin/SimpleXLS.php' );
		if ( $xls = stafflistUtils\SimpleXLS::parse( $file ) ) {
			$arr = $xls->rows();
		} else {
			echo stafflistUtils\SimpleXLS::parseError();
			$arr = false;
		}

	   	//importing from (.xlsx) format
	} elseif(substr(strtolower($file), -5) == '.xlsx'){
	   	
		require_once( STAFFLIST__PLUGIN_DIR . '/lib/shuchkin/SimpleXLSX.php' );
		if ( $xlsx = stafflistUtils\SimpleXLSX::parse( $file ) ) {
			$arr = $xlsx->rows();
		} else {
			echo stafflistUtils\SimpleXLSX::parseError();
			$arr = false;
		}
		...
		$format = 'csv';
		require_once( STAFFLIST__PLUGIN_DIR . '/lib/shuchkin/SimpleCSV.php' );
		if ( $arr = stafflistUtils\SimpleCSV::import( $file ) ) {

		} else {
			echo "Could not import from CSV.";
			$arr = false;
		}
...
function ajax_sort(){
	...
	$hidden = isset($_POST['hidden'])?$_POST['hidden']:array();
	if(!empty($hidden)){	
		foreach($hidden as $k=>$col){
			$id = str_replace("col", "", $col);
			$q = "UPDATE $staffmetadb SET colpos=%d, active=%d WHERE id=%d";
			$pq = $wpdb->prepare( $q, $pos, 0, $id ); $wpdb->query( $pq );
			$pos++;
		}
	}
}
...
function insert_stafflist( $atts ) {
	...
	//default sort
	if(isset($atts['sort']) && 
		is_numeric($atts['sort']) && 
		$wpdb->get_var($wpdb->prepare("SELECT active FROM {$staffmetadb} WHERE id=%d", (int) $atts['sort']))==1){
			$sort = (int) $atts['sort']; //shortcode sort=#
	} elseif(!$sort = $wpdb->get_var("SELECT id FROM {$staffmetadb} WHERE active>0 ORDER BY colpos ASC LIMIT 1")){
		$sort = 1;
	}
	...
	//search on
	if(isset($atts['on']) && in_array($atts['on'],array("enter","type","wait"))) $on=$atts['on'];
	...
	return("<div class='staffwrapper on{$on}'>
				<div class='pagerblock'>
					<div class='stafflistctl'>
						<input type='hidden' rel='sl_sort' value='{$sort}' autocomplete='Off'>
...
function handleStafflistExports(){
	...
	//build query
	$q =   		"SELECT * FROM {$staffdb} {$where} ORDER BY {$sort} {$dir}"; //echo $q;
	$sql = 		$wpdb->prepare($q,$params);
	$staff = 	$wpdb->get_results($sql, ARRAY_A);									 		//myprint_r($staff);
	...
	$filename = date("Ymd").'_StaffList';
	if($_GET['export']=="csv"){
		require_once( STAFFLIST__PLUGIN_DIR . '/lib/shuchkin/SimpleCSV.php' );
		$csv = stafflistUtils\SimpleCSV::export( $arr );
		header('Content-Type: text/csv');
		header('Content-Disposition: attachment;filename="' . $filename . '.csv"');
		exit( $csv );
	} else {
		require_once( STAFFLIST__PLUGIN_DIR . '/lib/shuchkin/SimpleXLSXGen.php' );
		$xlsx = stafflistUtils\SimpleXLSXGen::fromArray( $arr );
